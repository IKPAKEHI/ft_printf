...
static void		in0(t_flags **m, va_list *l, char **n, unsigned long int *a)
{
	if ((*m)->type == 1)
...
static void		in1(t_flags **mody, char **num, char **to_free, int *r_l)
{
	if ((*num)[1] != '\0')
	while (*(*num) == '0')
		(*num)++;
...
static void		in2(t_flags **m, char **num, char **res, int *r_l )
{
	if (((*num)[0] == '-' || (*m)->flags[1] == 1) && (*m)->precision_len >= (int)ft_strlen((*num)))
	{
